...
module.exports = {
  ...
  validarCamposUpdate: function (
    ...
    // Se reservaAtual existe, verifica se houve alguma alteração real
    if (reservaAtual) {
      const mesmosCampos =
        ...
        JSON.stringify(reservaAtual.dias_semana.split(",").map(String).sort()) ===
          JSON.stringify(dias_semana.map(String).sort()) &&
      ...
      if (mesmosCampos) {
        return {
          error:
            "Os dados informados são iguais aos da reserva periódica atual. Nenhuma alteração foi feita.",
        };
      }
    }
  ...
  validarConflitoReservaPeriodica: async function (
    fk_id_sala,
    data_inicio,
    data_fim,
    dias_semana,
   ...
   validarConflitoReservaPeriodicaUpdate: async function (
    id_reservaperiodica,
    fk_id_sala,
    data_inicio,
    data_fim,
    ...
    // Consulta reservas periódicas, exceto a que está sendo atualizada
    const queryReservasPeriodicas = `
      SELECT id_reservaperiodica, data_inicio, data_fim, dias_semana, hora_inicio, hora_fim
      FROM reservaperiodica
      WHERE fk_id_sala = ?
        AND data_fim >= ?
        AND data_inicio <= ?
        AND id_reservaperiodica != ?
    `;
    ...
    const reservasPeriodicas = await new Promise((resolve, reject) => {
      connect.query(queryReservasPeriodicas, [fk_id_sala, data_inicio, data_fim, id_reservaperiodica], (err, results) => {
        if (err) return reject(err);
        resolve(results);
      });
    });
    ...
    // Verifica conflitos em outras reservas periódicas
    for (const rp of reservasPeriodicas) {
      const diasRP = rp.dias_semana.split(",").map(Number);
      if (!diasRP.some((d) => diasSet.has(d))) continue;
      ...
      if (hInicio < rpFim && hFim > rpInicio) {
        conflitos.push({
          tipo: "reserva_periodica",
          id_reservaperiodica: rp.id_reservaperiodica,
          data_inicio: rp.data_inicio,
          data_fim: rp.data_fim,
          dias_semana: rp.dias_semana,
          hora_inicio: rp.hora_inicio,
